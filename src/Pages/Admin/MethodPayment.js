...
import { MethodPaymentEvent } from "./MethodPaymentEvent";
...
export const MethodPayment = () => {
  ...
  const getPayment = async (userID, eventAll) => {
    try {
      const response = await fetch(
        `http://localhost:3001/admin/method_payment/methodOrderTable`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
      ...
      const methodWithId = data.map((method) => ({
        ...
        orderID: eventAll.find((event) => event.orderID == method.orderID)
          ?.orderName,
      }));
  ...
  const orderDrop = orderData.map((value) => ({
    value: value.orderID,
    label: value.orderName,
  }));

  const updateMethod = async () => {
    try {
      const response = await fetch(
        "http://localhost:3001/admin/method_payment/methodTable/editMethod",
        {
          method: "PATCH",
          headers: {
            Accept: "application/json",
  ...
  const AddMethod = async () => {
    try {
      const response = await fetch(
        "http://localhost:3001/admin/method_payment/methodTable/addMethod",
        {
          method: "POST",
          headers: {
            Accept: "application/json",
  ...
  const handleEdit = (row) => {
    setEditMethod({
      orderID: orderDrop.find((e) => e.label === row.orderID)?.value,
  ...
  return (
    <div>
      ...
      <div className={styles.mainPageContainer}>
        <h1 className={styles.mainTitle}>Methods Payment Tables</h1>
        ...
        >
          <h2 style={{ color: "#161855" }}>Ticket Table</h2>
          <Paper
            component="form"
            sx={{
              p: "2px 4px",
      ...
      <FormDialogue
        show={openDialog}
        acceptLabel={"Save"}
        dialogueTitle={"Edit Ticket Payment"}
      ...
      >
        <div className={styles.dialogueContainer}>
          <Dropdown
            ...
            options={orderDrop}
            onChange={(value) =>
              setEditMethod({ ...editMethod, orderID: value })
            }
            value={editMethod.orderID}
            nullable
          />
          <Dropdown
      ...
      <FormDialogue
        show={openAddDialog}
        acceptLabel={"Save"}
        dialogueTitle={"Add Ticket Payment"}
      ...
      >
        <div className={styles.dialogueContainer}>
          ...
          <Dropdown
            placeholder={"Order"}
            options={orderDrop}
            onChange={(value) => setAddMethod({ ...addMethod, orderID: value })}
            value={addMethod.orderID}
            nullable
          />
          <Dropdown
            placeholder={"Payment For"}
